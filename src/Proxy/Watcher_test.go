...
func Test_WatcherGetRouteJson(t *testing.T) {
	w := &Watcher{}
	js, err := w.GetRouteJson()
	if err != nil {
		t.Error(err)
	} else {
		t.Log(js)
...
func Test_WatcherMap(t *testing.T) {
	m := make(map[string]*teststr)
	for i := 0; i < 10; i++ {
		tt := &teststr{"name" + strconv.Itoa(i), i}
		m["name"+strconv.Itoa(i)] = tt
	}
	t.Log(m)
